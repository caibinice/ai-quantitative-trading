...
export function DataQuality() {
  ...
  return <>
    ...
    <section className="panel table-panel issue-panel"><div className="panel-head"><div><span className="section-kicker">ACTIVE ALERTS</span><h2>数据质量告警</h2></div><button className="icon-button" onClick={load}><RefreshCcw size={16} /></button></div><div className="table-scroll"><table><thead><tr><th>级别</th><th>对象</th><th>分类</th><th>问题</th><th>最近发现（北京时间）</th><th>详情</th><th>操作</th></tr></thead><tbody>{issues.length ? issues.map((issue) => <tr key={issue.id}><td><span className={`severity ${issue.severity}`}>{issue.severity === 'critical' ? '严重' : issue.severity === 'warning' ? '警告' : '提示'}</span></td><td>{issue.entity_id}</td><td>{issue.category}</td><td><strong>{issue.title}</strong></td><td>{formatBeijingDateTime(issue.last_seen_at)}</td><td className="issue-detail">{JSON.stringify(issue.detail)}</td><td><button className="text-button" onClick={() => resolve(issue.id)}>标记解决</button></td></tr>) : <tr><td colSpan={7}><div className="empty-inline">当前没有活跃告警</div></td></tr>}</tbody></table></div></section>
  </>
}
