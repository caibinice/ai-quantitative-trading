...
export function DataQuality() {
  ...
  const benchmarkOption = useMemo(() => ({ tooltip: { trigger: 'axis' }, grid: { left: 58, right: 18, top: 20, bottom: 34 }, xAxis: { type: 'category', boundaryGap: false, data: benchmark.map((item) => item.date), axisLabel: { color: '#718096' }, axisLine: { lineStyle: { color: '#2a3b50' } } }, yAxis: { type: 'value', scale: true, axisLabel: { color: '#718096' }, splitLine: { lineStyle: { color: '#192a3b' } } }, dataZoom: [{ type: 'inside', start: Math.max(0, 100 - 12000 / Math.max(1, benchmark.length)), end: 100 }], series: [{ type: 'line', showSymbol: false, data: benchmark.map((item) => item.close), lineStyle: { color: '#a989ff', width: 2 }, areaStyle: { color: '#a989ff12' } }] }), [benchmark])
