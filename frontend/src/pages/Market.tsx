...
export function Market() {
  ...
  const priceSource = latest?.source === 'tushare-pro' ? 'Tushare Pro' : latest?.source ?? tr('多源数据')
  ...
  const chartOption = useMemo(() => ({
    ...
    series: [
      ...
      {
        name: tr('成交量'), type: 'bar', xAxisIndex: 1, yAxisIndex: 1,
  ...
  return (
    <>
      <PageHeader eyebrow="Market & fundamentals" title={tr("行情与财务显微镜")} description={tr("日线采用前复权价格；财务数据按报告期保留，便于核对来源与变化。")} />
      <section className="market-toolbar panel">
        <label className="select-wrap"><Search size={17} /><select value={symbol} onChange={(event) => setSymbol(event.target.value)}>
          {stocks.map((stock) => <option value={stock.symbol} key={stock.symbol}>{stock.symbol} · {tr(stock.name)}</option>)}
        </select></label>
        {selected && <div className="symbol-title"><strong>{tr(selected.name)}</strong><span>{selected.symbol} · {selected.market} {tr("股")}</span></div>}
        ...
        <div className="data-tag"><CalendarDays size={15} />{tr("更新至")} {latest?.date ?? '—'}</div>
      </section>
      {loading ? <Loading /> : !prices.length ? (
        <div className="empty-block tall"><TrendingUp /><strong>{tr("暂无行情数据")}</strong><span>{tr("到“策略实验室”同步研究数据（Tushare 优先、公开源降级）或运行演示数据脚本。")}</span></div>
      ) : (
        <section className="market-grid">
          <article className="panel price-panel"><div className="panel-head"><div><span className="section-kicker">PRICE ACTION</span><h2>{tr("历史行情")}</h2></div><span className="source-chip">{priceSource} {tr("· 前复权")}</span></div><ReactECharts key={theme} option={chartOption} style={{ height: 460 }} /></article>
          <aside className="panel quote-panel">
            <div className="panel-head"><div><span className="section-kicker">SNAPSHOT</span><h2>{tr("当日切片")}</h2></div></div>
            <div className="quote-grid">
              {[['开盘', latest?.open], ['最高', latest?.high], ['最低', latest?.low], ['收盘', latest?.close], ['成交量', latest?.volume], ['换手率', latest?.turnover_rate]].map(([label, value]) => (
                <div key={String(label)}><span>{tr(String(label))}</span><strong>{formatNumber(value as number, label === '成交量' ? 0 : 2)}</strong></div>
              ))}
            </div>
            <p className="small-note">{tr("前复权用于保持价格序列连续。网页数据存在延迟或源站调整风险，重要研究请交叉验证。")}</p>
          </aside>
        </section>
      )}
      <section className="panel financial-panel">
        <div className="panel-head"><div><span className="section-kicker">FUNDAMENTALS</span><h2>{tr("最新财务指标")}</h2></div><span className="source-chip">{financialSource ? `${financialSource} · ` : ''}{tr("报告期")} {latestReport ?? '—'}</span></div>
        {latestMetrics.length ? <div className="metric-grid">{latestMetrics.map((metric) => (
          <div className="metric-item" key={metric.metric_name}><span>{tr(metric.metric_name)}</span><strong>{formatNumber(metric.metric_value)}</strong><small className={(metric.yoy ?? 0) >= 0 ? 'up' : 'down'}>{metric.yoy === null ? tr('同比 —') : localize({ 'zh-CN': `同比 ${metric.yoy > 0 ? '+' : ''}${metric.yoy.toFixed(2)}%`, en: `YoY ${metric.yoy > 0 ? '+' : ''}${metric.yoy.toFixed(2)}%`, ja: `前年比 ${metric.yoy > 0 ? '+' : ''}${metric.yoy.toFixed(2)}%` })}</small></div>
        ))}</div> : <div className="empty-inline">{tr("暂无财务数据")}</div>}
      </section>
    </>
  )
}
