...
export function Tasks() {
  ...
  return <>
    ...
    ].map(([type, label, detail]) => <button key={type} onClick={() => create(type)} disabled={!!creating}><span><Play size={15} /></span><div><strong>{creating === type ? '正在入队…' : label}</strong><small>{detail}</small></div></button>)}</div><p className="quick-task-note">这四个按钮分别创建独立的单项任务，彼此不会自动串联。只想更新舆情和 AI 选股排名，请使用上方“立即执行全流程”。</p></section>
