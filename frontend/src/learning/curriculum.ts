...
export const learningStages: LearningStage[] = [
  {
    ...
    title: '建立地图',
    subtitle: '先知道自己在做什么',
    goal: '分清量化、AI 量化、Web3 与传统软件系统，读懂项目全貌。',
  },
  {
    id: 2,
    title: '掌握数据语言',
    subtitle: '从 Java / JS 迁移到科学 Python',
    goal: '用 Python、NumPy 和 pandas 正确表达收益率、窗口和时间序列。',
  },
  {
    id: 3,
    title: '构建可信策略',
    subtitle: '因子、交易成本与回测',
    goal: '实现有经济含义、没有未来数据、能和真实基准比较的策略。',
  },
  {
    id: 4,
    title: '加入 AI 与验证',
    subtitle: '舆情因子和样本外实验',
    goal: '把新闻转成可审计信号，并用 Walk-forward 检查过拟合。',
  },
  {
    id: 5,
    title: '完成研究系统',
    subtitle: '工程化与毕业项目',
    goal: '独立运行数据流水线、治理质量、解释结果并交付研究报告。',
  },
]
...
const pythonResources: LearningResource[] = [
  {
    ...
    note: '适合有其他语言经验的开发者快速查语法与标准库。',
  },
  {
    title: 'CS50’s Introduction to Programming with Python',
    provider: 'Harvard OpenCourseWare',
    url: 'https://cs50.harvard.edu/python/',
    note: '免费课程，结构是讲解、练习、测试和最终项目。',
  },
]
...
for daily_return in returns:
  ...
  },
  {
    ...
    duration: '3–4 小时',
    level: '入门',
    objective: '不急着写策略，先能启动系统、识别每一层职责并追踪一次请求。',
    outcomes: [
      '能说明 React、FastAPI、Worker、MySQL 和外部数据源的职责。',
      '能从前端按钮追踪到 API、任务队列和服务函数。',
      '能运行演示数据并分辨 demo 与真实数据。',
    ],
    concepts: [
      {
        title: 'Java / JS 开发者的架构映射',
        summary: 'FastAPI 路由类似 Controller，SQLAlchemy 模型类似 Entity，service 模块承载领域逻辑。',
        points: [
          'Pydantic Schema 类似带运行时校验的 DTO。',
          'React 页面只负责交互与展示，不在浏览器里执行回测。',
          'Worker 消费持久化任务，避免长任务阻塞 HTTP 请求。',
        ],
      },
      {
        title: '一次任务的生命周期',
        summary: '前端只负责入队，Worker 认领任务、更新进度、执行服务并保存结果。',
        points: [
          'queued → running → success / failed / cancelled。',
          '任务 payload 是可审计的输入快照。',
          '失败可重试，但要避免把非幂等副作用执行两次。',
        ],
      },
      {
        title: '研究数据分层',
        summary: '原始行情、点时财务、新闻、情绪分析、因子分数和实验结果分别保存。',
        points: [
          '原始数据尽量保留 source 与日期。',
          '派生结果需要参数、模型版本和生成时间。',
          '展示层不应偷偷修改研究数据。',
  ...
  → React polling`,
    ...
    quiz: [
      {
        question: '为什么外部数据同步不直接在前端请求中完成？',
        options: ['React 不能发 HTTP', '长任务需要持久化、重试和进度', 'MySQL 只能由 Worker 访问'],
        answer: 1,
        explanation: '任务队列让长流程脱离请求生命周期，并保留状态与失败信息。',
      },
      {
        question: 'Pydantic Schema 最接近 Java 中的什么概念？',
        options: ['DTO + 校验', 'JVM 字节码', 'CSS Module'],
        answer: 0,
        explanation: 'Schema 描述输入输出形状并在运行时验证。',
      },
      {
        question: '研究结果可审计至少需要保留什么？',
        options: ['只保留最终收益', '参数、数据时间和生成上下文', '只保留截图'],
        answer: 1,
        explanation: '没有输入和时间上下文，结果无法复现或排查未来数据。',
      },
    ],
    resources: [
      {
  ...
  },
  {
    ...
    duration: '1 周',
    level: '基础',
    objective: '能够阅读和修改项目 Python 代码，并写出有类型提示、可测试的小函数。',
    outcomes: [
      '理解动态类型、可变对象、切片、推导式、生成器和上下文管理器。',
      '能正确使用虚拟环境、模块、异常和 pytest。',
      '知道 Python 循环与 NumPy/pandas 向量化的边界。',
    ],
    concepts: [
      {
        title: '语法不是难点，语义才是',
        summary: '重点关注引用语义、可变默认参数、None、truthiness 与 duck typing。',
        points: [
          'list / dict 可变，tuple / str 不可变。',
          'is 比较身份，== 比较值；None 通常用 is None。',
          '类型提示用于工具和可读性，默认不会像 Java 编译器一样强制。',
        ],
      },
      {
        title: 'Pythonic 数据处理',
        summary: '推导式、zip、enumerate、迭代器和 dataclass 能减少样板代码。',
        points: [
          '先追求明确，再追求一行表达式。',
          '大型数据计算不应逐行 Python for 循环。',
          'with 管理数据库 Session、文件和锁的生命周期。',
        ],
      },
      {
        title: '测试与错误阅读',
        summary: '把 traceback 从最后一行向上读，先定位异常类型和自己的代码入口。',
        points: [
          'pytest 用普通函数和 assert 即可开始。',
          '纯函数最容易构造边界样例。',
          '金融代码要显式测试空数据、NaN、错位和极端值。',
        ],
      },
    ],
    checklist: [
      '用 Python 重写一个熟悉的 Java Stream 操作。',
      '解释 list、tuple、dict、set 的选择。',
      '写一个带类型提示和 docstring 的收益率函数。',
      '制造并读懂一次 IndexError traceback。',
      '运行 backend 的 pytest。',
      '完成本章测验且得分不低于 2/3。',
    ],
    projectFiles: [
      { path: 'backend/app/schemas.py', reason: '观察类型提示、默认值和校验器。' },
      { path: 'backend/app/services/scoring.py', reason: '阅读小型纯函数和 NumPy 计算。' },
      { path: 'backend/tests/test_scoring.py', reason: '学习 pytest 的 Arrange–Act–Assert。' },
    ],
    demo: {
      file: 'learning/examples/01_python_bridge.py',
      command: '.\\.venv\\Scripts\\python.exe learning\\examples\\01_python_bridge.py',
      summary: '对比 Java Stream、JavaScript map/reduce 与 Python 推导式。',
    ...
    quiz: [
      {
        question: 'Python 类型提示的默认行为是什么？',
        options: ['运行前强制编译检查', '提升可读性和工具检查，但通常不阻止运行', '自动转换错误类型'],
        answer: 1,
        explanation: 'mypy/编辑器可静态检查，但解释器默认不执行完整类型校验。',
      },
      {
        question: '判断变量是否为 None 推荐使用什么？',
        options: ['value == None', 'value is None', 'Boolean(value)'],
        answer: 1,
        explanation: 'None 是单例，使用身份比较更清晰且符合 Python 风格。',
      },
      {
        question: '处理百万行收益计算优先选择什么？',
        options: ['嵌套 Python 循环', 'pandas/NumPy 向量化', '递归'],
        answer: 1,
        explanation: '向量化通常更快，也更贴合时间序列算子的表达。',
      },
    ],
